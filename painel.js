...
if (btnEditarInfo) {
  btnEditarInfo.onclick = async () => {
    ...
    let imagemUrlFinal = ponto.imagem_url || "";

    const desejaTrocarImagem = window.confirm("Deseja trocar a imagem deste ponto?");
    if (desejaTrocarImagem) {
      try {
        ...
        if (arquivo) {
          ...
          imagemUrlFinal = await uploadImagemPonto(arquivo, codigoSelecionado);
        }
      } catch (error) {
        console.error(error);

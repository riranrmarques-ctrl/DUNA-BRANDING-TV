...
function ajustarTituloPontoEmUmaLinha() {
  if (!nomePontoDetalhe) return;

  nomePontoDetalhe.style.fontSize = "";
  nomePontoDetalhe.style.whiteSpace = "nowrap";

  const tamanhoInicial = parseFloat(window.getComputedStyle(nomePontoDetalhe).fontSize);
  let tamanhoAtual = tamanhoInicial;

  while (nomePontoDetalhe.scrollWidth > nomePontoDetalhe.clientWidth && tamanhoAtual > 28) {
    tamanhoAtual -= 2;
    nomePontoDetalhe.style.fontSize = `${tamanhoAtual}px`;
  }
}
...
function renderizarDetalheBase(ponto, historico) {
  ...
  if (nomePontoDetalhe) {
    ...
    requestAnimationFrame(ajustarTituloPontoEmUmaLinha);
  }
  ...
  if (statusDesdeDetalhe) {
    ...
    statusDesdeDetalhe.textContent = dataUltimoRegistro
      ? `último registro: ${formatarDataHora(dataUltimoRegistro)}`
      : "ainda sem registro";
  }
}

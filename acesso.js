...
function obterDuracaoVirtualPreview(item, tipo = "") {
  ...
  if (tipoFinal === "imagem") return 8000;
  if (tipoFinal === "site") return 12000;
...
function ordenarHistoricoAscendente(historico = []) {
  return [...(historico || [])].sort((a, b) => {
    const dataA = new Date(obterDataHistorico(a)).getTime() || 0;
    const dataB = new Date(obterDataHistorico(b)).getTime() || 0;
    return dataA - dataB;
  });
}

function obterInicioPeriodoAtivo(ponto, historico = []) {
  const status = calcularStatusPonto(ponto, historico);
  if (status.classe !== "ativo") return null;

  const ordenado = ordenarHistoricoAscendente(historico);
  let inicio = null;

  ordenado.forEach((item) => {
    const evento = obterEventoHistorico(item);
    const data = new Date(obterDataHistorico(item));

    if (Number.isNaN(data.getTime())) return;

    if (eventoEhInativo(evento)) inicio = null;
    if (eventoEhAtivo(evento)) inicio = data;
  });

  if (inicio) return inicio;

  const ultimoRegistro = historico[0] || null;
  const fallback = obterDataHistorico(ultimoRegistro) || obterUltimoPingPonto(ponto);
  const dataFallback = new Date(fallback);

  return Number.isNaN(dataFallback.getTime()) ? null : dataFallback;
}
...
function calcularReproducoesVirtuaisDoCliente(playlist = [], ponto, historico = []) {
  ...
  const tempoAtivoMs = calcularTempoAtivoMs(ponto, historico);
  if (tempoAtivoMs <= 0) return 0;

  const duracaoCicloMs = lista.reduce((total, item) => total + obterDuracaoMaterialMs(item), 0);
  if (duracaoCicloMs <= 0) return 0;

  const ciclosCompletos = Math.floor(tempoAtivoMs / duracaoCicloMs);
  const resto = tempoAtivoMs % duracaoCicloMs;
  ...
  lista.forEach((item) => {
    ...
    if (itemPertenceAoCliente(item)) {
      total += ciclosCompletos;

      if (resto >= acumulado + duracao) {
        total += 1;
      }
    }
...
function calcularReproducoesVirtuaisTotaisCliente() {
  return pontosContratados.reduce((total, ponto) => {
    const codigo = normalizarCodigo(ponto.codigo);
    const playlist = playlistsPorPonto[codigo] || [];
    const historico = historicosPorPonto[codigo] || [];

    return total + calcularReproducoesVirtuaisDoCliente(playlist, ponto, historico);
  }, 0);
}

function aplicarReproducoesVirtuaisNaSerie(serie = []) {
  const hoje = obterHojeISO();
  const totalVirtual = calcularReproducoesVirtuaisTotaisCliente();

  return serie.map((dia) => ({
    ...dia,
    total: Number(dia.total || 0) + (dia.iso === hoje ? totalVirtual : 0)
  }));
}
...
function renderizarPreview(lista, indice = null, statusPonto = null) {
  ...
  if (!url) {
    ...
    previewMidia.innerHTML = `
      <video src="${escapeHtml(url)}" ${offline ? "" : "autoplay"} muted playsinline></video>
      ${avisoOffline}
    `;
  }
...
async function carregarPlaylistsIniciais(pontos) {
  const pares = await Promise.all(
    pontos.map(async (ponto) => {
      const codigo = normalizarCodigo(ponto.codigo);

      try {
        const playlist = await buscarPlaylistPonto(codigo);
...
async function abrirPonto(codigo) {
  ...
  try {
    const [playlist, historico] = await Promise.all([
      playlistAtual.length ? Promise.resolve(playlistAtual) : buscarPlaylistPonto(codigoNormalizado),
...
async function carregarAreaCliente(codigo) {
  ...
  try {
    ...
    await carregarPlaylistsIniciais(pontosContratados);
